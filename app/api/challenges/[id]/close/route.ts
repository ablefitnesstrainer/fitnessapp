...
export async function POST(request: Request, { params }: { params: { id: string } }) {
  ...
  if (role === "coach" && challenge.created_by !== userId) {
    return NextResponse.json({ error: "Coach can only close own challenges" }, { status: 403 });
  }

  const { error: updateError } = await supabase.from("challenges").update({ status: "closed" }).eq("id", params.id);
